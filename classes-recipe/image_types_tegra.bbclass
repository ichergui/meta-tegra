...
tegraflash_custom_sign_pkg() {
    if [ -n "${TEGRA_SIGNING_ARGS}" ]; then
        ${TEGRA_SIGNING_ENV} MACHINE=${TNSPEC_MACHINE} ./tegra-flash-helper.sh --no-flash ${TEGRA_SIGNING_ARGS} flash.xml.in ${LNXFILE} ${IMAGE_BASENAME}.${IMAGE_TEGRAFLASH_FS_TYPE}
